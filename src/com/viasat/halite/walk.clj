...
;; This file was brought in from clojure and the patch from
;; https://clojure.atlassian.net/browse/CLJ-2568 was applied to support providing a function to
;; preserve metadata
...
the sorting function."}
 com.viasat.halite.walk)

(defn walk*
  "Same as walk, but outer receives old form as first argument and new form as 
second argument."
  {:added "1.12"}
  [inner outer form]
  (cond
    ...
    (outer form (clojure.lang.MapEntry/create (inner (key form)) (inner (val
                                                                         form))))
    (seq? form) (outer form (doall (map inner form)))
    (instance? clojure.lang.IRecord form)
    (outer form (reduce (fn [r x] (conj r (inner x))) form form))
...
(defn walk
  ...
  {:added "1.1"}
  [inner outer form]
  (walk* inner (fn [_old new] (outer new)) form))

(defn postwalk*
  "Same as postwalk, but outer receives old and new form as arguments."
  {:added "1.12"}
  [f form]
