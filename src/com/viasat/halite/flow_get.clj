...
(s/defn ^:private push-down-get* :- (s/maybe bom/Expr)
  [path :- [s/Any]
   field
   expr :- bom/Expr]
  (cond
...
(s/defn push-down-get :- (s/maybe bom/Expr)
  [path :- [s/Any]
   field
   expr :- bom/Expr]
  (let [result (push-down-get* path field expr)]
    (when trace
      (pprint/pprint [:push-down-get expr :path path :field field :result result]))
    result))
