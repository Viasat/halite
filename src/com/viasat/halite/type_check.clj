...
(s/defn type-check-instance :- types/HaliteType
  ...
  (let [t (or (:$type inst)
              (format-errors/throw-err (h-err/missing-type-field {error-key inst})))
        _ (when-not (types/namespaced-keyword? t)
            (format-errors/throw-err (h-err/invalid-type-value {error-key inst})))
        spec-info (or (envs/lookup-spec (:senv ctx) t)
                      (format-errors/throw-err (h-err/resource-spec-not-found {:spec-id (symbol t)
                                                                               error-key inst})))
    ...
    (when (seq missing-vars)
      (format-errors/throw-err (h-err/missing-required-vars {:missing-vars (mapv symbol missing-vars) :form inst})))
    (when (seq invalid-vars)
      (format-errors/throw-err (h-err/field-name-not-in-spec {:invalid-vars (mapv symbol invalid-vars)
                                                              :instance inst
    ...
    (doseq [[field-kw field-val] (dissoc inst :$type)]
      (let [field-type (get field-types field-kw)
        ...
        (when-not (types/subtype? actual-type field-type)
          (format-errors/throw-err (h-err/field-value-of-wrong-type {error-key inst
                                                                     :variable (symbol field-kw)
                                                                     :expected field-type
                                                                     :actual actual-type})))
        (when (and (map? field-val)
                   (:abstract? (envs/lookup-spec (:senv ctx) (:$type field-val))))
          (format-errors/throw-err (h-err/no-abstract {:value field-val
                                                       :field field-kw
...
(s/defn type-check-coll :- types/HaliteType
  ...
  (let [elem-types (map (partial check-fn ctx) coll)
    ...
    (when (not coll-type-string)
      (format-errors/throw-err (h-err/invalid-collection-type {error-key coll})))
    (doseq [[elem elem-type] (map vector coll elem-types)]
      (when (types/maybe-type? elem-type)
        (format-errors/throw-err (h-err/literal-must-evaluate-to-value {:coll-type-string (symbol coll-type-string)
                                                                        error-key elem}))))
...
(s/defn ^:private type-check-fn-application :- types/HaliteType
  ...
  (let [[op & args] form
    ...
    (when (nil? builtin)
      (format-errors/throw-err (h-err/unknown-function-or-operator {:op op
                                                                    :form form})))
    (loop [[sig & more] signatures]
      (cond
        (nil? sig) (format-errors/throw-err (h-err/no-matching-signature {:op op
                                                                          :form form
...
(s/defn ^:private type-check-symbol :- types/HaliteType
  ...
  (if (= '$no-value sym)
    ...
    (or (envs/lookup-type* (:tenv ctx) sym)
        (format-errors/throw-err (h-err/undefined-symbol {:form sym})))))

(defn- arg-count-exactly
  [n form]
  (when (not= n (count (rest form)))
    (format-errors/throw-err (h-err/wrong-arg-count {:op (first form)
...
(defn- arg-count-at-least
  ...
  (when (< (count (rest form)) n)
    (format-errors/throw-err (h-err/wrong-arg-count-min {:op (first form)
                                                         :minimum-arg-count n
                                                         :actual-arg-count (count (rest form))
                                                         :form form}))))
...
(defn- arg-count-odd
  ...
  (when (not (= (mod (count (rest form)) 2) 1))
    (format-errors/throw-err (h-err/wrong-arg-count-odd {:op (first form)
                                                         :actual-arg-count (count (rest form))
                                                         :form form}))))
...
(defn ^:private type-check-lookup [ctx form subexpr-type index]
  ...
  (cond
    ...
    (do
      (when (keyword? index)
        (format-errors/throw-err (h-err/invalid-vector-index {:form form :index-form index, :expected :Integer})))
      (let [index-type (type-check* ctx index)]
        (when (not= :Integer index-type)
          (format-errors/throw-err (h-err/invalid-vector-index {:form form :index-form index, :expected :Integer, :actual-type index-type})))
    ...
    (let [field-types (->> subexpr-type types/spec-id (envs/lookup-spec (:senv ctx)) :fields)]
      (when-not (types/bare-keyword? index)
        (format-errors/throw-err (h-err/invalid-instance-index {:form form, :index-form index})))
      (when-not (contains? field-types index)
        (format-errors/throw-err (h-err/field-name-not-in-spec {:form form, :index-form index, :spec-id (symbol (types/spec-id subexpr-type)) :invalid-vars [(symbol index)]})))
      (get field-types index))

    :else (format-errors/throw-err (h-err/invalid-lookup-target {:form form, :actual-type subexpr-type}))))
...
(s/defn ^:private type-check-get-in :- types/HaliteType
  ...
  (let [[_ subexpr indexes] form]
    (when-not (vector? indexes)
      (format-errors/throw-err (h-err/get-in-path-must-be-vector-literal {:form form})))
...
(defn add-position [n m]
  (let [m' (assoc m
                  :position-text (format-errors/text (condp = n
                                                       nil "Argument"
                                                       0 "First argument"
                                                       1 "Second argument"
                                                       "An argument")))]
...
(s/defn ^:private type-check-rescale :- types/HaliteType
  ...
  (let [[_ _ scale] expr
    ...
    (when-not (types/decimal-type? (first arg-types))
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 0 {:op 'rescale :expected-type-description (format-errors/text "a fixed point decimal") :expr expr}))))
    (when-not (= :Integer (second arg-types))
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 1 {:op 'rescale :expected-type-description (format-errors/text "an integer") :expr expr}))))
    (when-not (base/integer-or-long? scale)
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 1 {:op 'rescale :expected-type-description (format-errors/text "an integer literal") :expr expr}))))
    (when-not (and (>= scale 0)
                   (< scale (inc fixed-decimal/max-scale)))
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 1 {:op 'rescale
                                                                         :expected-type-description (format-errors/text (format "an integer between 0 and %s" fixed-decimal/max-scale))
                                                                         :expr expr}))))
    (if (zero? scale)
      :Integer
...
(s/defn ^:private type-check-if :- types/HaliteType
  ...
  (let [[pred-type s t] (mapv (partial type-check* ctx) (rest expr))]
    (when (not= :Boolean pred-type)
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 0 {:op 'if
                                                                         :actual-type pred-type
                                                                         :expected-type-description (format-errors/text "boolean") :expr expr}))))
    (types/meet s t)))
...
(s/defn ^:private type-check-cond :- types/HaliteType
  ...
  (loop [[pred-type t & more] (mapv (partial type-check* ctx) (rest expr))
    ...
    (if t
      (do
        (when (not= :Boolean pred-type)
          (format-errors/throw-err (h-err/arg-type-mismatch (add-position 0 {:op 'cond :expected-type-description (format-errors/text "boolean") :expr expr}))))
        (recur more (if s
                      (types/meet s t)
                      t)))
...
(s/defn ^:private type-check-when :- types/HaliteType
  ...
  (let [[pred-type body-type] (map (partial type-check* ctx) (rest expr))]
    (when (not= :Boolean pred-type)
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 0 {:op 'when :expected-type-description (format-errors/text "boolean") :expr expr}))))
...
(s/defn ^:private type-check-let :- types/HaliteType
  ...
  (let [[bindings body] (rest expr)]
    (when-not (zero? (mod (count bindings) 2))
      (format-errors/throw-err (h-err/let-bindings-odd-count {:form expr})))
    (type-check*
     (reduce
      (fn [ctx [sym body]]
        (when-not (types/bare-symbol? sym)
          (format-errors/throw-err (h-err/let-needs-bare-symbol {:form expr})))
...
(s/defn ^:private type-check-comprehend
  ...
  (let [[op [sym expr :as bindings] body] expr]
    (when-not (= 2 (count bindings))
      (format-errors/throw-err (h-err/comprehend-binding-wrong-count {:op op :form expr})))
    (when-not (types/bare-symbol? sym)
      (format-errors/throw-err (h-err/binding-target-must-be-bare-symbol {:op op :form expr :sym sym})))
    (let [coll-type (type-check* ctx expr)
          et (types/elem-type coll-type)
          _ (when-not et
              (format-errors/throw-err (h-err/comprehend-collection-invalid-type {:op op, :expr-type coll-type, :form expr
...
(s/defn ^:private type-check-quantifier :- types/HaliteType
  ...
  (when (not= :Boolean (:body-type (type-check-comprehend ctx expr)))
    (format-errors/throw-err (h-err/not-boolean-body {:op (first expr)
                                                      :form expr})))
  :Boolean)
...
(s/defn ^:private type-check-filter :- types/HaliteType
  ...
  (let [{:keys [coll-type body-type]} (type-check-comprehend ctx expr)]
    (when (not= :Boolean body-type)
      (format-errors/throw-err (h-err/not-boolean-body {:op 'filter
                                                        :form expr})))
    coll-type))
...
(s/defn ^:private type-check-sort-by :- types/HaliteType
  ...
  (let [{:keys [coll-type body-type]} (type-check-comprehend ctx expr)]
    (when-not (or (= :Integer body-type)
      ...
      (format-errors/throw-err (h-err/not-sortable-body {:op 'sort-by :form expr :actual-type body-type})))
...
(s/defn ^:private type-check-reduce :- types/HaliteType
  ...
  (let [[op [acc init] [elem coll] body] expr]
    (when-not (types/bare-symbol? acc)
      (format-errors/throw-err (h-err/accumulator-target-must-be-bare-symbol {:op op, :accumulator acc, :form expr})))
    (when-not (types/bare-symbol? elem)
      (format-errors/throw-err (h-err/element-binding-target-must-be-bare-symbol {:op op, :form expr, :element elem})))
    (when (= acc elem)
      (format-errors/throw-err (h-err/element-accumulator-same-symbol {:form expr, :accumulator acc, :element elem})))
    (let [init-type (type-check* ctx init)
          coll-type (type-check* ctx coll)
          et (types/elem-type coll-type)]
      (when-not (types/subtype? coll-type (types/vector-type :Value))
        (format-errors/throw-err (h-err/reduce-not-vector {:form expr, :actual-coll-type coll-type})))
...
(s/defn ^:private type-check-if-value :- types/HaliteType
  ...
  (let [[op sym set-expr unset-expr] expr]
    ...
    (when-not (types/bare-symbol? sym)
      (format-errors/throw-err (h-err/if-value-must-be-bare-symbol {:op op
                                                                    :form expr})))
...
(s/defn ^:private type-check-if-value-let :- types/HaliteType
  ...
  (let [[op [sym maybe-expr] then-expr else-expr] expr]
    ...
    (when-not (types/bare-symbol? sym)
      (format-errors/throw-err (h-err/binding-target-must-be-bare-symbol {:op op
                                                                          :sym sym})))
...
(defn check-all-sets [[op :as expr] arg-types]
  (when-not (every? #(types/subtype? % (types/set-type :Value)) arg-types)
    (format-errors/throw-err (h-err/arguments-not-sets {:op op, :form expr}))))
...
(s/defn ^:private type-check-first :- types/HaliteType
  ...
  (let [arg-type (type-check* ctx (second expr))]
    (when-not (or (types/subtype? arg-type (types/vector-type :Value))
      ...
      (format-errors/throw-err (h-err/argument-not-collection {:op 'first, :form expr})))
...
(s/defn ^:private type-check-rest :- types/HaliteType
  ...
  (let [arg-type (type-check* ctx (second expr))]
    (when-not (types/subtype? arg-type (types/vector-type :Value))
      (format-errors/throw-err (h-err/argument-not-vector {:op 'rest, :form expr})))
    arg-type))
...
(s/defn ^:private type-check-conj :- types/HaliteType
  ...
  (let [[base-type & elem-types] (mapv (partial type-check* ctx) (rest expr))]
    (when-not (types/subtype? base-type (types/coll-type :Value))
      (format-errors/throw-err (h-err/argument-not-set-or-vector {:form expr})))
    (doseq [[elem elem-type] (map vector (drop 2 expr) elem-types)]
      (when (types/maybe-type? elem-type)
        (format-errors/throw-err (h-err/cannot-conj-unset {:type-string (symbol (types/coll-type-string base-type)), :form elem}))))
...
(s/defn ^:private type-check-concat :- types/HaliteType
  ...
  (let [op (first expr)
    ...
    (when-not (types/subtype? s (types/coll-type :Value))
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 0 {:op op :expected-type-description (format-errors/text "a set or vector"), :form expr}))))
    (when-not (types/subtype? t (types/coll-type :Value))
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 1 {:op op :expected-type-description (format-errors/text "a set or vector"), :form expr}))))
    (when (and (types/subtype? s (types/vector-type :Value)) (not (types/subtype? t (types/vector-type :Value))))
      (format-errors/throw-err (h-err/not-both-vectors {:op op, :form expr})))
    (types/meet s
...
(s/defn ^:private type-check-refine-to :- types/HaliteType
  ...
  (let [[subexpr kw] (rest expr)
    ...
    (when-not (types/subtype? s (types/instance-type))
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 0 {:op 'refine-to :expected-type-description (format-errors/text "an instance"), :form expr, :actual s}))))
    (when-not (types/namespaced-keyword? kw)
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 1 {:op 'refine-to :expected-type-description (format-errors/text "a spec id"), :form expr}))))
    (when-not (envs/lookup-spec (:senv ctx) kw)
      (format-errors/throw-err (h-err/resource-spec-not-found {:spec-id (symbol kw) :form expr})))
...
(s/defn ^:private type-check-refines-to? :- types/HaliteType
  ...
  (let [[subexpr kw] (rest expr)
        s (type-check* ctx subexpr)]
    (when-not (types/subtype? s (types/instance-type))
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 0 {:op 'refines-to? :expected-type-description (format-errors/text "an instance"), :form expr}))))
    (when-not (types/namespaced-keyword? kw)
      (format-errors/throw-err (h-err/arg-type-mismatch (add-position 1 {:op 'refines-to? :expected-type-description (format-errors/text "a spec id"), :form expr}))))
    (when-not (envs/lookup-spec (:senv ctx) kw)
      (format-errors/throw-err (h-err/resource-spec-not-found {:spec-id (symbol kw) :form expr})))
    :Boolean))
...
(s/defn ^:private type-check-valid :- types/HaliteType
  ...
  (let [t (type-check* ctx subexpr)]
    (cond
      ...
      :else (format-errors/throw-err (h-err/arg-type-mismatch (add-position nil {:op 'valid, :expected-type-description (format-errors/text "an instance of known type"), :form expr}))))))
...
(s/defn type-check* :- types/HaliteType
  ...
  (let [type (cond
    ...
    (when (and (or (types/halite-vector-type? type)
      ...
      (format-errors/throw-err (h-err/unknown-type-collection {:form expr})))
    type))
...
(s/defn type-check-constraint-expr
  ...
  (let [t (type-check* {:senv senv :tenv tenv} bool-expr)]
    (when (not= :Boolean t)
      (format-errors/throw-err (h-err/not-boolean-constraint {:type t})))))
...
(s/defn type-check-refinement-expr
  ...
  (let [expected-type (types/maybe-type (types/concrete-spec-type spec-id))
    ...
    (when-not (types/subtype? t expected-type)
      (format-errors/throw-err (h-err/invalid-refinement-expression {:form expr
