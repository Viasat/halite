...
(deftest application-tests
  ...
  (are [expr err-msg]
    ...
    '(sort-by [1 5] 1) #"must be a symbol"
    '(reduce [0 1] [x []] 2) #"must be a symbol"
    '(reduce [a 1] [2 []] 3) #"must be a symbol"
...
(deftest maybe-tests
  (let [senv (assoc-in senv [:specs :ws/Maybe$v1] {:spec-vars {:x [:Maybe "Integer"]}})
    ...
    (are [expr err-msg]
      ...
      '(if-value 12 true false) #"must be a symbol"
      '(when-value 12 true) #"must be a symbol"
